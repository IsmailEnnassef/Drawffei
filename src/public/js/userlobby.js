...
class History extends React.Component{
    ...
    componentDidMount(){
        fetch('http://127.0.1.1:3001/users/history',{method:'GET',
    headers:new Headers({
    ...
    render(){
        const visual = this.state.history.map((name)=>{
            return <li>
                    {name}
            </li>
        })
        return( <div id="history">
            <h4>history of games you've created</h4>
            <ul>
                {visual}
            </ul>
...
class Lobby extends React.Component{
    ...
    renderDestroy(name){
        return(
            <button id={this.getId(name)+'_destroy'} onClick={(e)=>{this.destroy(name)}}>Supprimer {name}</button>
        )
    }
    ...
    render(){
        ...
        const Available=roomListe.map((roomJson)=>{
            ...
            return(
                <li class="list-group-item roombuttons userButtons" >
        ...
        return(<div class='lobby'>
            ...
            <ol>
            <div>
        <div class="">pick a game !</div>
        <div>
            <ol class="list-group"> {Available}</ol>
        </div>
      </div>
            </ol>
            <History/>
